...
def main():
    global args, best_src_val_acc, best_tgt_val_acc
    ...
    if args.dataset_name is None or args.dataset_name == 'imagenet':
        ...
        val_datasets.append([val_dataset, 'val', True])
        ...
        train_dataset = Breeds(
                ...
                **args.dataset_kwargs)
        src_val_dataset = Breeds(
                args.data_path, split='val',
                source=True, target=False,
                **args.dataset_kwargs)
        tgt_val_dataset = Breeds(
                args.data_path, split='val',
        ...
        src_val_dataset._transform = get_val_transform(
            train_dataset.means, train_dataset.stds
        )
        tgt_val_dataset._transform = get_val_transform(
            train_dataset.means, train_dataset.stds
        )
        val_datasets.append([src_val_dataset, 'src_val', False])
        val_datasets.append([tgt_val_dataset, 'tgt_val', True])
        logger, training_stats = initialize_exp(
            args, "epoch", "loss", "prec1", "prec5", "loss_src_val", "prec1_src_val", "prec5_src_val",
            "loss_tgt_val", "prec1_tgt_val", "prec5_tgt_val"
        )
        ...
        src_val_dataset = DomainNet(
            source_domain, split='test',
            **args.dataset_kwargs
        )
        tgt_val_dataset = DomainNet(
    ...
    val_loaders = []
    for (val_dataset, label, should_track) in val_datasets:
        ...
        val_loaders.append([val_loader, label, should_track])
    ...
    if args.dataset_name == 'imagenet':
        num_classes = 1000
    else: # TODO: why is this returning wrong number?????? What number is it returning?
    ...
    to_restore = {"epoch": 0, "best_src_val_acc": 0., "best_tgt_val_acc": 0.}
    restart_from_checkpoint(
    ...
    best_src_val_acc = to_restore["best_src_val_acc"]
    best_tgt_val_acc = to_restore["best_tgt_val_acc"]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        stats = []
        stats.extend([train(model, linear_classifier, optimizer, train_loader, epoch)])
        for val_loader, label, should_track in val_loaders:
            # because of the ordering defined above, this will preserve the ordering so
            # the column names from training_stats will be correct
            stats.extend([validate_network(val_loader, model, linear_classifier, label, should_track)])
...
def validate_network(val_loader, model, linear_classifier, label, should_track):
    # label is for the logging information, what this val loader is
    # should_track is a boolean for whether or not `best_acc` should be updated
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    if should_track and (top1.avg.item() > best_acc):
    ...
    if args.rank == 0:
        logger.info(
            ...
            "Best Acc@1 so far {acc:.1f}".format(
                label=label, batch_time=batch_time, loss=losses, top1=top1, acc=best_acc))
